...
public class TopicMapTopic extends LiveTopic {
	...
	public CorporateDirectives executeCommand(String command, Session session,
		...
		// --- create associaion ---
			...
			directives.add(as.createAssociation(assocID, ASSOCTYPE_GENERIC, topicID1, topicID2));
		...
		// --- publish ---
			...
			String workgroupID = st.nextToken();
			directives.add(publish(workgroupID, session));
		// --- import ---
	...
	public CorporateDirectives propertiesChanged(Hashtable newData, Hashtable oldData,
		...
		// --- "Background Image" ---
		String prop = (String) newData.get(PROPERTY_BACKGROUND_IMAGE);
		if (prop != null) {
		...
		// --- "Background Color" ---
		prop = (String) newData.get(PROPERTY_BACKGROUND_COLOR);
		if (prop != null) {
		...
		// --- "Icon" ---
		prop = (String) newData.get(PROPERTY_ICON);
		if (prop != null) {
	...
	private String openGroupView(Session session, CorporateDirectives directives, BaseTopic owner)
		...
		String workgroupID = owner.getID();
		...
		if (session.isDemo()) {
			...
			cm.createAssociation(as.getNewAssociationID(), 1, SEMANTIC_ORIGIN_GROUP, 1, personalViewID, 1, workgroupID, 1);
			...
			directives.add(DIRECTIVE_SHOW_VIEW, personalViewMetadata, topicmap,
				session.getPersonalWorkspace().getID(), workgroupID);
		}
		//
	...
	/**
	 * Returns the client directives for publishing this topicmap to the specified workgroup.
	 *
	 * @see		#executeChainedCommand
	 */
	private CorporateDirectives publish(String workgroupID, Session session) throws DeepaMehtaException {
		CorporateDirectives directives = new CorporateDirectives();
		// cause the client to remove the topicmap topic from the users personal workspace
		directives.add(DIRECTIVE_HIDE_TOPIC, getID(), Boolean.FALSE, /* ### die=false */
			session.getPersonalWorkspace().getID());
		// --- add document upload directives ---
		addPublishDirectives(directives);
		boolean isFirstPublishing = as.getOriginWorkgroup(getID()) == null;
		// --- publish this map ---
		publishTo(workgroupID, isFirstPublishing);
		// --- create topic in shared workspace ---
		if (isFirstPublishing) {
			// adding DIRECTIVE_SHOW_TOPIC causes the client to creating a new
			// topicmap topic in the shared workspace
			PresentableTopic topic = new PresentableTopic(this);
			// ### topic.setFreeGeometry();
			directives.add(DIRECTIVE_SHOW_TOPIC, topic, Boolean.FALSE,
				as.getWorkspace(workgroupID).getID());
		}
		// --- close view ---
		if (as.isViewOpen(getID(), session.getUserID())) {
			close(session, directives);
		}
		// --- notify all workgroup members ---
		CorporateDirectives notify = new CorporateDirectives();
		// ### workgroup name needed
		String notifyText = isFirstPublishing ?
			"Topicmap \"" + getName() + "\" has been published to workgroup \"" +
			workgroupID + "\" by user \"" + session.getUserName() + "\"" :
			"Topicmap \"" + getName() + "\" (workgroup \"" + workgroupID + "\") has " +
			"been updated by user \"" + session.getUserName() + "\"";
		notify.add(DIRECTIVE_SHOW_MESSAGE, notifyText, new Integer(NOTIFICATION_DEFAULT));
		Vector users = as.workgroupMembers(workgroupID);
	 ...
	 * @see		#publish
	 */
	private void publishTo(String workgroupID, boolean isFirstPublishing) {
		if (isFirstPublishing) {
			System.out.println("> TopicMapTopic.publishTo(): publish " + this + " to \"" + workgroupID +
			...
			System.out.println("> TopicMapTopic.publishTo(): publish " + this + " to \"" +
				workgroupID + "\" (supersedes " + originTopicmap + " of this group)");
			// delete origin topicmap
			as.deleteUserView(originTopicmapID, originTopicmapVersion);
			// override this topicmap with origin topicmap
