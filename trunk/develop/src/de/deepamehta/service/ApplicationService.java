...
public final class ApplicationService extends BaseTopicMap implements Runnable, LoginCheck, DeepaMehtaConstants {
	 ...
	 *
	 * References checked: 28.3.2003 (2.0a18-pre8)
	 *
	 * @see		de.deepamehta.topics.TopicMapTopic#executeCommand
	 */
	public CorporateDirectives createAssociation(String assocID, String typeID, String topicID1, String topicID2) {
		CorporateDirectives directives = new CorporateDirectives();
	 ...
	 * References checked: 28.3.2003 (2.0a18-pre8)
	 *
	 * @see		de.deepamehta.assocs.LiveAssociation#executeCommand		CMD_CHANGE_ASSOC_TYPE_BY_NAME
	 */
	 ...
	 *																Session session, CorporateDirectives directives)
	 * @see		de.deepamehta.assocs.LiveAssociation#executeCommand		CMD_CHANGE_ASSOC_TYPE
	 */
	public CorporateDirectives changeAssociationType(String assocID, int version, String typeID, int typeVersion,
		...
		String assocType1 = topic1.associationAllowed(typeID, topic2.getID(), directives);
		String assocType2 = topic2.associationAllowed(typeID, topic1.getID(), directives);
		//
		boolean allowed1 = assocType1 != null;
		boolean allowed2 = assocType2 != null;
		//
		...
		//
		boolean retype1 = !assocType1.equals(typeID);
		boolean retype2 = !assocType2.equals(typeID);
		//
		if (retype1 && retype2) {
			...
			System.out.println(">>> " + topic1 + " modified the assoc retyping (\"" + typeID + "\" -> \"" + assocType1 + "\")");
			typeID = assocType1;
		} else if (retype2) {
			System.out.println(">>> " + topic2 + " modified the assoc retyping (\"" + typeID + "\" -> \"" + assocType2 + "\")");
			typeID = assocType2;
		}
		// --- trigger associated() hook ---
		topic1.associated(typeID, assoc.getType(), topic2.getID(), session, directives);
		topic2.associated(typeID, assoc.getType(), topic1.getID(), session, directives);
		// --- trigger associationRemoved() hook ---
		topic1.associationRemoved(assoc.getType(), topic2.getID(), session, directives);
		topic2.associationRemoved(assoc.getType(), topic1.getID(), session, directives);
		//
	...
	private void updateServerConsole() {
		if (component instanceof ServerConsole) {
			((ServerConsole) component).updateSessions();
		}
	}
	 ...
	 */
	public Component getGraphicsContext() {
		return component;
	}
	 ...
	 * @see		DeepaMehta#init
	 */
	public void setGraphicsContext(Component component) {
	...
	/**
	 * References checked: 25.3.2004 (2.0b3-pre2)
	 *
	 ...
	 * @see		de.deepamehta.topics.TypeTopic#addButton							false
	 */
