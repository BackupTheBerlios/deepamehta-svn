...
class RelationalCorporateMemory implements CorporateMemory, DeepaMehtaConstants {
	...
	*/
	...
	/**
	 * Private unified form of latter 10 getRelatedTopics() forms.
	 * <P>
	...
	public Vector getRelatedViewTopics(String topicmapID, int version, String topicID, String assocTypeID,
	...
	public void setTopicData(String topicID, int version, String field, String value) {
		...
		if (field.length() == 0) {
			System.out.println("*** RelationalCorporateMemory.setTopicData(): " +
				topicID + ":" + version + " property name is empty (value: \"" + value +
				"\") -- property not stored");
			return;
		}
		// ### this check is performed due to oracle compatibility
		// ### -- discarded because setting empty is necessary if a property value
		// exists already
		/* if (value.length() == 0) {
			System.out.println("> RelationalCorporateMemory.setTopicData(): " +
				topicID + ":" + version + " value is empty (property: \"" + field +
				"\") -- property not stored");
			return;
		} */
		if (LOG_CM) {
			System.out.println("> (>) " + topicID + ":" + version + " (update \"" +
				field + "\" to \"" + value + "\")");
		}
		//
		int rowCount = update("UPDATE TopicProp SET PropValue='" + quote(value) +
			"' WHERE TopicID='" + topicID + "' AND TopicVersion=" + version + " AND " +
			"PropName='" + quote(field) + "'", IMPLIED_NO);
		if (rowCount == 0) {
			// the topic has no topic data yet -- insert
			if (LOG_CM) {
				System.out.println("> (#) " + topicID + ":" + version + " (insert \"" +
					field + "\" to \"" + value + "\")");
			}
			update("INSERT INTO TopicProp " +
				"(TopicID, TopicVersion, PropName, PropValue) VALUES " +
				"('" + topicID + "', " + version + ", '" + quote(field) + "', '" +
					quote(value) + "')");
		} else if (rowCount > 1) {
			System.out.println("*** RelationalCorporateMemory.setTopicData(): " +
				rowCount + " rows updated, \"" + topicID + "\", " + version + ", \"" +
				field + "\"");
		}
	}
