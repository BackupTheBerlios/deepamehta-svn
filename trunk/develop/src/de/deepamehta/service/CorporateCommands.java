...
public final class CorporateCommands extends Commands implements DeepaMehtaConstants {
	...
	/* ### public void addSearchByPropertyCommand(Session session, CorporateDirectives directives) {
		addCommand(as.string(ITEM_SEARCH_BY_PROPERTY), CMD_CREATE_TOPIC + ":" + "tt-corporatesearch",
			FILESERVER_IMAGES_PATH, ICON_SEARCH_BY_PROPERTY);
	} */
	 ...
	 * ### to be dropped
	 * <P>
	 * Adds a "Rename" command to this (topic) command set.
	 *
	 * @see		#addStandardCommands
	 */
	/* private void addRenameTopicCommand(BaseTopic topic, Session session) {
		String item = as.string(topic.getName().equals("") ? ITEM_SET_TOPIC_NAME : ITEM_CHANGE_TOPIC_NAME);
		addCommand(item, CMD_CHANGE_TOPIC_NAME, FILESERVER_IMAGES_PATH, ICON_CHANGE_TOPIC_NAME);
	} */
