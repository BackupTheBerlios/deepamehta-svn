...
public final class PresentationService implements DeepaMehtaConstants,
	...
	Component createErrorGUI(Throwable e, String host, int port) {
		...
		if (runsAsDemo) {
			...
			if (host != null && e instanceof IOException) {
				...
				p.add(new JLabel("The DeepaMehta server at " + (host.equals("") ? "port " : host + ":") + port));
				addTextToPanel("isn't running\" (" + e + ")", p, 42);
			} else {
				addTextToPanel(e.toString(), p, 42);
			}
		}
		//
		return p;
	...
	private void createMessagePanel() {
		...
		// ### new JScrollPane(messagePanel, JScrollPane.VERTICAL_SCROLLBAR_NEVER,
		// ###	JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		//
		// ### ImageIcon editorIcon = getIcon(FILESERVER_IMAGES_PATH, NOTIFICATION_ICONS[0]);		// ###
		// ### topicmapChoice.addItem(new ComboBoxItem(editorIcon, string(MESSAGE_LABEL), null));	// topicID=null
		//
		/* ### addToUpperArea(0, DEFAULT_BGCOLOR_MESSAGE, string(MESSAGE_LABEL),
			messagePanel.getEmptyIcon(), new JScrollPane(messagePanel,
	...
	private void addTextToPanel(String text, Container p, int charWidth) {
		...
		if (text.length() > charWidth) {
			while ((pos = text.lastIndexOf(' ', charWidth)) != -1) {
